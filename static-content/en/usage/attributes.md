...
Publixx PIM supports eight data types that determine the input field and validation:
...
<svg viewBox="0 0 800 440" xmlns="http://www.w3.org/2000/svg" style="max-width:100%;height:auto;margin:1.5rem 0;">
  ...
  <text class="dt-desc" x="226" y="168">counts, and whole numbers.</text>
  ...
  <text class="dt-desc" x="226" y="286">Choice from a value list.</text>
  ...
  <text class="dt-desc" x="420" y="286">Key-value pairs as JSON</text>
  <text class="dt-desc" x="420" y="300">structure. For flexible</text>
  ...
  <text class="dt-desc" x="614" y="286">Structured collections as</text>
  <text class="dt-desc" x="614" y="300">JSON array. For repeatable</text>
  <text class="dt-desc" x="614" y="314">data blocks.</text>
...
#### String
For single-line and multi-line text. Ideal for product names, descriptions, short texts, and technical designations. String attributes can be marked as **translatable**, so a separate text is maintained per language.

#### Number
Stores integers without decimal places. Suitable for quantity specifications, item counts, stock values, and integer metrics.

#### Float
Floating-point numbers with configurable precision. Can be linked to a **unit** from a unit group (e.g., weight in kg, length in mm). The unit is displayed next to the input field.
...
#### Collection
JSON arrays with repeatable structures. Allows storing lists of similar data records within a single attribute. Application example: multiple certification entries, each with name, number, and validity date.
...
| **Unique** (`is_unique`) | The value must be unique system-wide (e.g., for EAN numbers) |
...
| **Variant Attribute** (`is_variant_attribute`) | Marks the attribute as variant-specific (distinguishes variants from each other) |
...
::: tip Best Practice
Choose the technical name carefully -- it serves as an API key and cannot be changed afterwards. Use descriptive, English snake_case identifiers (e.g., `net_weight`, `short_description`).
:::

## Attribute Groups (AttributeTypes)

Attribute groups -- referred to as **AttributeTypes** in the system -- are used for the logical organization of attributes. They group thematically related attributes together and determine their order in the product detail view.

### Management

Via the **Attribute Groups** menu item in the sidebar, you can access the group overview. There you can:

- **Create a new group** -- Assign a technical name and display names in DE and EN.
- **Assign attributes** -- Assign existing attributes to the group and set the order.
- **Edit groups** -- Change the display name or the order of contained attributes.
...
Unit groups combine physical units of a category. They are assigned to attributes of type **Float**, so the user can select the appropriate unit when filling in values.
...
Value lists define the available options for attributes of type **Selection**. You can access the management via the **Value Lists** menu item in the sidebar.
...
Attribute views define **subsets** of available attributes for specific usage contexts. They allow restricting the display and access to attributes by intended purpose.
...
Attributes can have a parent-child relationship with each other. A **parent attribute** serves as a logical container for its **child attributes**, which are only displayed when the parent attribute has a specific value.
...
This concept reduces the complexity of product maintenance by only displaying contextually relevant attributes.
