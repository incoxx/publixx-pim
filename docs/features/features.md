...
| **CMS Integration** | Self-service guide + tools (starter kit, CORS/reverse-proxy config, scoped service token) for embedding the catalog-embed widget into any external CMS (TYPO3, WordPress, Drupal, …) without an iFrame. See `docs/features/typo3-cms-integration.md`. | Agencies get a ready-to-run blueprint and a one-click starter kit instead of custom integration work per project. |
