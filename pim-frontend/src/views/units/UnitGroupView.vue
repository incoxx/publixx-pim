...
async function fetchGroups() {
  loading.value = true
  try {
    const { data } = await unitGroups.list({
      include: 'units',
...
<template>
  <div class="flex gap-6 h-full">
    ...
    <div :class="['space-y-4 transition-all', selectedGroup ? 'w-1/3 flex-none' : 'flex-1']">
      ...
      <PimFilterBar :search="search" placeholder="Einheitengruppen durchsuchen..." @update:search="v => { search = v; meta.current_page = 1; fetchGroups() }" />
      <PimTable
        :columns="columns"
        :rows="items"
        :loading="loading"
      ...
      >
        <template #pagination>
          <div class="flex items-center justify-between px-4 py-3 border-t border-[var(--color-border)]">
            <span class="text-xs text-[var(--color-text-tertiary)]">{{ meta.total }} Einheitengruppen</span>
