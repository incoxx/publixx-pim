...
const ROLES = ['image', 'title', 'subtitle', 'price', 'badge', 'attribute', 'rating', 'cta']
const TYPES = ['text', 'price', 'media_url', 'media_array', 'unit_value', 'composite']
const PREFIXES = ['product:', 'attribute:', 'prices:', 'media:', 'relations:', 'collection:']
...
function newWidget() {
  ...
  current.value = {
    ...
    is_system: false,
    is_active: true,
    config: { fields: [], show_sku: false, image_ratio: '4/3', cta: { enabled: false, label: '' } },
  }
}

function addField() {
  current.value.config.fields.push({ role: 'attribute', source: 'attribute:', type: 'text' })
}
function removeField(idx) {
  current.value.config.fields.splice(idx, 1)
}

// Drag-Sortierung der Felder
...
const fieldError = computed(() => Object.keys(errors.value).find(k => k.startsWith('config.fields')))
...
<template>
  <div class="space-y-4" data-testid="product-widgets">
    ...
    <p class="text-[11px] text-[var(--color-text-tertiary)] -mt-2">
      Definiert, welche Produktdaten ein Produktblock zeigt. Getrennt von der Katalog-Vorschau-Konfiguration.
    </p>
      ...
      <div v-if="current" class="md:col-span-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4 space-y-3">
        ...
        <div>
          <div class="flex items-center justify-between mb-1">
            <label class="text-[12px] font-medium text-[var(--color-text-secondary)]">Felder (Rolle → Quelle)</label>
            <button class="pim-btn pim-btn-secondary text-[11px]" @click="addField"><Plus class="w-3 h-3" :stroke-width="2" /> Feld</button>
          </div>
          <p v-if="fieldError" class="mb-1 text-[11px] text-[var(--color-error)]">Bitte Rolle, Quelle und Typ für jedes Feld angeben.</p>
          <div class="space-y-1.5">
            ...
            <datalist id="widget-source-prefixes">
              <option v-for="p in PREFIXES" :key="p" :value="p" />
            </datalist>
            <p v-if="!current.config.fields.length" class="text-xs text-[var(--color-text-tertiary)]">Noch keine Felder. „Feld" hinzufügen.</p>
          </div>
        </div>
