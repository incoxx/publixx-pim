...
function getIcon(type) {
  const icons = { text: Type, image: Image, shape: Square, variant_table: Table2, relation_table: Network, attribute_table: List, smart_table: LayoutGrid }
    ...
    <div>
      ...
      <div v-if="expandedGroups.layout && store.availableFields?.layout_elements" class="space-y-0.5">
        <div
          ...
          class="flex items-center gap-2 px-2 py-1 rounded text-[11px] hover:bg-[var(--color-bg)] text-[var(--color-text-secondary)] cursor-grab"
          draggable="true"
          @dragstart="onDragStart($event, { type: el.type, ...(el.type === 'text' ? { content: 'Text hier eingeben' } : {}), ...(el.type === 'image' ? { source: 'primary' } : {}), ...(el.type === 'relation_table' ? { columns: ['sku', 'name'], relationTypeId: null, productAttributeIds: [] } : {}), ...(el.type === 'attribute_table' ? { sourceMode: 'group', attributeGroupId: null, attributeIds: [] } : {}), ...(el.type === 'smart_table' ? { bind: 'variants', ptl: {} } : {}) })"
          @dblclick="onDoubleClick({ type: el.type, ...(el.type === 'text' ? { content: 'Text hier eingeben' } : {}), ...(el.type === 'image' ? { source: 'primary' } : {}), ...(el.type === 'relation_table' ? { columns: ['sku', 'name'], relationTypeId: null, productAttributeIds: [] } : {}), ...(el.type === 'attribute_table' ? { sourceMode: 'group', attributeGroupId: null, attributeIds: [] } : {}), ...(el.type === 'smart_table' ? { bind: 'variants', ptl: {} } : {}) })"
        >
          <GripVertical class="w-3 h-3 text-[var(--color-text-tertiary)]" :stroke-width="1.5" />
