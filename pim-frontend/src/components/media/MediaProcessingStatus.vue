...
async function fetchStatus() {
  try {
    ...
    if (status.value?.pdf?.active || status.value?.av?.active) {
...
<template>
  <Transition name="slide-down">
    ...
    >
      ...
      <!-- Audio/Video-Verarbeitung aktiv -->
      <template v-else-if="status.av?.active">
        <Loader2 class="w-4 h-4 animate-spin text-[var(--color-primary)]" :stroke-width="2" />
        <div class="flex items-center gap-2 flex-1">
          <Video class="w-3.5 h-3.5 text-[var(--color-text-tertiary)]" :stroke-width="2" />
          <span class="text-[var(--color-text-primary)]">
            Audio/Video-Verarbeitung:
            <strong v-if="status.av.processing > 0">{{ status.av.processing }} aktiv</strong>
            <template v-if="status.av.pending > 0">
              <span v-if="status.av.processing > 0">, </span>
              {{ status.av.pending }} wartend
            </template>
          </span>
          <span v-if="status.av.recently_ready > 0" class="text-[var(--color-success)]">
            <CheckCircle class="w-3 h-3 inline" /> {{ status.av.recently_ready }} fertig
          </span>
          <span v-if="status.av.recent_errors > 0" class="text-[var(--color-error)]">
            <AlertCircle class="w-3 h-3 inline" /> {{ status.av.recent_errors }} Fehler
          </span>
        </div>
      </template>

      <!-- Nur kürzlich fertige Audio/Video-Verarbeitungen (kein aktiver Job) -->
