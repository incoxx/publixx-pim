...
// Re-fetch facets when locale or folder/other filters change
watch(() => store.locale, () => {
  store.fetchFacets()
})
...
watch(() => store.usagePurposeFilter, () => {
  store.fetchFacets()
})
watch(() => store.mediaTypeFilter, () => {
  store.fetchFacets()
})
watch(() => store.usageTypeFilter, () => {
  store.fetchFacets()
})
  ...
  <div v-if="store.facets.length" class="py-2">
    ...
    <div class="px-4 pb-2 flex items-center justify-between">
      ...
      <button
        ...
        @click="store.clearAllFilters(); store.fetchAssets(); store.fetchFacets()"
      >
